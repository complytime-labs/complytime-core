...
func ValidateTargetRegistration(reg *TargetRegistrationYAML) error {
	...
	for i, p := range reg.Target.TrustedPublishers {
		...
		if seen[key] {
			return fmt.Errorf("trusted-publishers[%d]: duplicate (%s, %s)", i, p.Issuer, p.SubPattern)
		}
		seen[key] = true
	}
	...
	for i, p := range reg.Target.RemovePublishers {
		...
		key := p.Issuer + "\x00" + p.SubPattern
		if seenRemove[key] {
			return fmt.Errorf("remove-publishers[%d]: duplicate (%s, %s)", i, p.Issuer, p.SubPattern)
		}
		if seen[key] {
