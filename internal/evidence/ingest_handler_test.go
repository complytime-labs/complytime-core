...
func TestValidateTargetRegistration_DuplicateWithinTrustedPublishers(t *testing.T) {
	reg := &TargetRegistrationYAML{}
	reg.Target.ID = "test-target"
	reg.Target.TrustedPublishers = []TrustedPublisherYAML{
		{Issuer: "https://example.com", SubPattern: "repo:*"},
	...
	if err := ValidateTargetRegistration(reg); err == nil {
		t.Fatal("expected error for duplicate within trusted-publishers")
	}
}

func TestValidateTargetRegistration_DuplicateWithinRemovePublishers(t *testing.T) {
	reg := &TargetRegistrationYAML{}
	reg.Target.ID = "test-target"
	reg.Target.RemovePublishers = []RemovePublisherYAML{
		{Issuer: "https://old.example.com", SubPattern: "old:*"},
		{Issuer: "https://old.example.com", SubPattern: "old:*"},
	}
	if err := ValidateTargetRegistration(reg); err == nil {
		t.Fatal("expected error for duplicate within remove-publishers")
	}
}
