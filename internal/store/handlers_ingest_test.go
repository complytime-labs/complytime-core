...
func TestIngestAsyncHandler_DSSEPassThrough(t *testing.T) {
	dsseBody := []byte(`{"payload":"dGVzdA==","payloadType":"application/vnd.in-toto+json","signatures":[{"sig":"abc123"}]}`)
	var captured []byte
	appender := &mockAppender{addFn: func(_ context.Context, data []byte) (uint64, error) {
		captured = data
		return 3, nil
	...
	handler := IngestAsyncHandler(publisher, tracker, appender, verifier, nil, authorizer)
