...
func TestResolvePublishAction_NoTargetID_ReturnsError(t *testing.T) {
	...
	_, _, _, err := resolvePublishAction(context.Background(), body, claims, nil, receipt.FormatYAML)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "artifact missing target.id")
}

func TestResolvePublishAction_DSSE_RequiresPublisherTrust(t *testing.T) {
	// DSSE payload: {"target":{"id":"tgt-1"}} base64-encoded
	payload := `eyJ0YXJnZXQiOnsiaWQiOiJ0Z3QtMSJ9fQ==`
	body := []byte(`{"payload":"` + payload + `","payloadType":"application/vnd.gemara+json","signatures":[{"sig":"abc"}]}`)
	...
	// No trusted publishers for tgt-1 — should resolve with publisher_trusted=false
	claims := &auth.JWTClaims{Iss: "https://issuer.example.com", Sub: "repo:evil/attacker"}
	action, attrs, targetID, err := resolvePublishAction(ctx, body, claims, store, receipt.FormatDSSE)
	assert.NoError(t, err)
	assert.Equal(t, "publish:artifact", action.ID.String())
	assert.Equal(t, cedar.Boolean(false), attrs["publisher_trusted"])
	assert.Equal(t, "tgt-1", targetID)

	// Add trusted publisher — should be allowed
	require.NoError(t, store.InsertTrustedPublishers(ctx, []requirements.TrustedPublisherRow{
		{TargetID: "tgt-1", Issuer: "https://issuer.example.com", SubPattern: "repo:org/*", AddedAt: time.Now()},
	}))

	claims = &auth.JWTClaims{Iss: "https://issuer.example.com", Sub: "repo:org/signer"}
	action, attrs, targetID, err = resolvePublishAction(ctx, body, claims, store, receipt.FormatDSSE)
	assert.NoError(t, err)
	assert.Equal(t, "publish:artifact", action.ID.String())
...
func TestResolvePublishAction_DSSE_NoTarget_ReturnsError(t *testing.T) {
	// DSSE payload with no target.id
	payload := `eyJ0eXBlIjoiRXZhbHVhdGlvbkxvZyJ9` // {"type":"EvaluationLog"}
	body := []byte(`{"payload":"` + payload + `","payloadType":"application/vnd.gemara+json","signatures":[{"sig":"abc"}]}`)
	claims := &auth.JWTClaims{Iss: "https://issuer.example.com", Sub: "anyone"}

	_, _, _, err := resolvePublishAction(context.Background(), body, claims, nil, receipt.FormatDSSE)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "artifact missing target.id")
}
